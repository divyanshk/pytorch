...
if TYPE_CHECKING:
    ...
    from torch.utils.data._utils import _BaseQueue, _MPQueue
...
class _ParallelDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __init__(self, loader):
        ...
        self._worker_result_queue: Optional[_BaseQueue] = None
        self._shutdown = False
        self._workers_done_event: Union[
            threading.Event, python_multiprocessing.synchronize.Event, None
        ] = None
        self._index_queues: list[Any] = []
        self._workers: list[Any] = []
        self._data_queue: Optional[_BaseQueue] = None
    ...
    def _shutdown_workers(self):
        ...
        try:
            if hasattr(self, "_pin_memory_thread"):
                if self._worker_result_queue is None:
                    raise AssertionError("Worker result queue not initialized")
                # Cast to _MPQueue since we know multiprocessing queues have these methods
                mp_queue: _MPQueue = self._worker_result_queue  # type: ignore[assignment]
